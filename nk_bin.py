...
class B000FF:
    ...
    def finalize(self):
        ...
        self._file.write(struct.pack(self.HEADER_FMT,
                                     self.MAGIC, self._start_addr, length))
...
class TOCEntry:
    ...
    def __init__(self,
        ...
        self.data = struct.pack(self.FMT, attrs, time,
                                size, name_ptr, e32_ptr, o32_ptr, load_ptr)
...
def main():
    ...
    # ROMHDR pointer
    b000ff.add_entry(START_ADDR + 0x40, b'ECEC' +
                     struct.pack('<I', ROMHDR_ADDR))
    # pTOC (relative pointer to ROMHDR). No idea whether this really needs to
    # be a separate entry
    b000ff.add_entry(START_ADDR + 0x48,
                     struct.pack('<I', ROMHDR_ADDR - START_ADDR))
